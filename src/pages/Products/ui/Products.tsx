...
const Products = () => {
    ...
    const { data, isLoading, error } = useQuery(
        productsQueryOptions({ page, perPage, category, sort })
    );

    function setNewPage(newPage: number) {
        updateSearchParams(
            { perPage: String(perPage), page: String(newPage) },
            searchParams,
            setSearchParams
        );
    }
                ...
                <div className={`flex flex-col gap-y-8 mt-8`}>
                    <div className={`flex items-center gap-4 mb-6 self-center`}>
                        ...
                        <SheetTrigger asChild>
                            <Button
                                variant="outline"
                                className={`border-amber-600 border-2 cursor-pointer relative`}
                            >
                                Корзина
                                <ShoppingCart />
                                {cartItemsCount > 0 && (
                                    <span
                                        className={`absolute -top-2 -right-2 bg-amber-600 text-white text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center`}
                                    >
